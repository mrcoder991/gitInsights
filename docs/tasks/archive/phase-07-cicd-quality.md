...
**Goal**: every PR is automatically typechecked, linted, and tested. Every merge to `main` deploys the app to GH Pages and the proxy to Vercel. The bundled holidays dataset is auto-refreshed yearly.
...
- Removed: Lighthouse CI step and `lighthouserc.json` dropped — not worth the maintenance cost against breaking Lighthouse releases.
